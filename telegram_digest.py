...
for msg in pending:
    ...
    if already_commented:
        print(f"Message {message_id} already commented, skipping.")
        processed_ids.append(message_id)   # mark as done even if comment exists
        continue
    ...
    if file_info and 'file_id' in file_info:
        ...
        if file_size <= MAX_FILE_SIZE:
            ...
            comment_body += f"[📎 Download {file_type}]({raw_url})\n"
        else:
            direct_link = f'https://api.telegram.org/file/bot{BOT_TOKEN}/{file_path}'
            comment_body += f"[📁 Large {file_type} (>{MAX_FILE_SIZE//1024//1024} MB)]({direct_link})\n"
